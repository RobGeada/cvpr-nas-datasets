...
def load_gutenberg(metainfo=False):
    ...
    for text in os.listdir('texts'):
        with open('texts/' + text, "r") as f:
            ...
            if author in ['aquinas', 'confucius', 'hawthorne', 'plato', 'shakespeare', 'tolstoy']:
    ...
    invalids = ['prologue', 'epilogue', 'chapter', 'scene', 'act', 'ii', 'iii', 'iv', 'v', 'vi', 'vii', 'viii', 'ix', 'x',
    ...
    for k, v in text_proc.items():
        ...
        for word in range(0, len(v) - phrase_size):
            ...
            if all([phrase_word_bounds[0] <= len(w) <= phrase_word_bounds[1] for w in phrase]):
